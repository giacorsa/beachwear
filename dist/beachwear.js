...
export class Cliente {
    ...
    ordinaProdotto(prodotto) {
        ...
        console.log(`Cliente ${this.nome} ${this.cognome} ha ordinato il prodotto ${prodotto.id} (${prodotto.tipo}, taglia ${prodotto.taglia}, colore ${prodotto.colore}).`);
    }
}
export class Prodotto {
    constructor(tipo, id, taglia, colore, stato = "disponibile") {
        this.tipo = tipo;
        this.id = id;
        this.taglia = taglia;
        this.colore = colore;
    ...
    assegnaCliente(cliente) {
        ...
        this.stato = "ordinato";
        console.log(`Prodotto ${this.id} assegnato al cliente ${cliente.nome} ${cliente.cognome}. Stato aggiornato a "${this.stato}".`);
    }
}
export class ProcessoProduzione {
    ...
    aggiungiProdotto(prodotto) {
        ...
        console.log(`Prodotto ${prodotto.id} (${prodotto.tipo}) aggiunto al processo "${this.nomeProcesso}".`);
    }
}
...
const costumeRelax = new Prodotto("costume da bagno", "RELAX-001", "M", "blu oceano", "disponibile");
prodotti.push(costumeRelax);
const costumeActive = new Prodotto("costume da bagno", "ACTIVE-010", "L", "verde alghe", "disponibile");
prodotti.push(costumeActive);
const costumeExtreme = new Prodotto("costume da bagno", "EXTREME-100", "S", "nero profondo", "esaurito");
prodotti.push(costumeExtreme);
const pareoSunrise = new Prodotto("pareo", "PAREO-050", "taglia unica", "corallo", "disponibile");
prodotti.push(pareoSunrise);
const cappelloWave = new Prodotto("cappello", "CAP-007", "taglia unica", "sabbia", "disponibile");
prodotti.push(cappelloWave);
const occhialiExtreme = new Prodotto("occhiali da sole", "OCC-009", "taglia unica", "nero", "disponibile");
prodotti.push(occhialiExtreme);
// Clienti del brand
const clienteGianni = new Cliente("Gianni", "Corsato", "gianni.corsato@example.com", "carta di credito");
const clienteSara = new Cliente("Sara", "Bianchi", "sara.bianchi@example.com", "paypal");
const clientePablo = new Cliente("Pablo", "Picasso", "pablo.picasso@artist.com", "cash");
// Processi di produzione sostenibile
const processoRicicloReti = new ProcessoProduzione("Riciclo Reti da Pesca", "Trasforma reti da pesca dismesse in filato riciclato per costumi Sunnee.");
processi.push(processoRicicloReti);
const processoTinturaSostenibile = new ProcessoProduzione("Tintura a Basso Impatto", "Utilizza coloranti a basso impatto ambientale e ridotto consumo d'acqua.");
...
prodotti.forEach((p) => {
    console.log(`ID: ${p.id}, tipo: ${p.tipo}, stato: ${p.stato}, cliente: ${p.clienteAssegnato
...
processi.forEach((proc) => {
    ...
    proc.prodottiInProduzione.forEach((p) => console.log(`  - ${p.id} (${p.tipo}) – stato: ${p.stato}`));
});
//# sourceMappingURL=beachwear.js.map